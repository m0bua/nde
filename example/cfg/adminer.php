        ...
        if ($function === 'loginForm') {
          ...
          echo strtr($form, [
            ...
            'name="auth[server]" value="'
              ...
              . '" title="hostname[:port]"',
            'name="auth[username]" id="username" value="" autocomplete="username"' =>
            'name="auth[username]" id="username" value="'
              . ($_ENV['ADMINER_DEFAULT_USER'] ?: 'root')
              . '" autocomplete="username"',
            'name="auth[password]" autocomplete="current-password"' =>
...
namespace {
  if (
    basename($_SERVER['DOCUMENT_URI'] ?? $_SERVER['REQUEST_URI']) === 'adminer.css'
